...
df_truth = pd.read_csv('Truth Dataset.csv')

# import dataset to be compared with truth
df1 = pd.read_csv('Other Dataset.csv')
...
for i in range(len(col_names)):
    # remove any data entries containing no data
    df_truth_clean = df_truth[col_names[i]].dropna()
    df1_clean = df1[col_names[i]].dropna()
    # convert cleaned data columns to lists
...
print(len(col_accuracy1), len(col_lengths), len(col_names_valid))
